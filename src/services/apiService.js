...
export const fetchStrategyDescription = async (strategyName) => {
  try {
    ...
    // Make the request for strategy description
    const response = await fetch(`${API_BASE_URL}/strategy_descritption`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const fetchAnalysisResult = async (parameters) => {
  try {
    // First check if we can connect to the backend
    const healthCheck = await fetch(`${API_BASE_URL}/data`);
    if (!healthCheck.ok) {
